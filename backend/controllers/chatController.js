...
export const getUsers = async (req, res) => {
  try {
    const users = await User.find({ _id: { $ne: req.user._id } }).select('_id name email avatar online lastActive').lean();
...
export const createIndividualChat = async (req, res) => {
  ...
  try {
    const recipient = await User.findById(recipientId).select('_id name avatar online lastActive').lean();
    if (!recipient) {
    ...
    }).populate('members', '_id name avatar online lastActive').lean();
    if (!chat) {
      chat = new Chat({
        type: 'individual',
      ...
      await chat.save();
      chat = await Chat.findById(chat._id).populate('members', '_id name avatar online lastActive').lean();
    }
...
export const getGroups = async (req, res) => {
  try {
    const groups = await Chat.find({
      ...
      .populate('members', '_id name avatar online lastActive')
...
export const createGroup = async (req, res) => {
  ...
  try {
    ...
    const validMembers = await User.find({ _id: { $in: uniqueMembers } }).select('_id name avatar online lastActive').lean();
...
export const searchMessages = async (req, res) => {
  ...
  try {
    ...
    const skip = (parseInt(page, 10) - 1) * parseInt(limit, 10);
    const messages = await Message.find({ chatId, content: { $regex: query, $options: 'i' } })
      .select('_id chatId sender content fileUrl fileName contentType createdAt isDeleted isEdited reactions replyTo readBy')
      .populate('sender', '_id name avatar')
      .populate('replyTo', '_id content sender')
      .sort({ createdAt: -1 })
      ...
      .limit(parseInt(limit, 10))
      .lean();
    const totalMessages = await Message.countDocuments({ chatId, content: { $regex: query, $options: 'i' } });
    const totalPages = Math.ceil(totalMessages / parseInt(limit, 10));
    res.json({
      success: true,
      messages: messages.reverse(),
    ...
    console.error('Error searching messages:', error.message, error.stack);
