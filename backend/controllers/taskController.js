...
export const createTask = async (req, res) => {
    try {
        ...
        // Enforce completed based on checklist
...
export const updateTask = async (req, res) => {
    try {
        const data = { ...req.body };
        delete data.completed; // Prevent client from setting completed manually
        if (data.dueDate) {
            data.dueDate = new Date(data.dueDate);
        }
        // Calculate completed based on checklist if provided
        if (data.checklist) {
            data.completed = data.checklist.length > 0 && data.checklist.every(i => i.checked);
        }
