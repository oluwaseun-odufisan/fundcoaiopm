...
Checklist: ${task.checklist?.length > 0 ? task.checklist.map(cl => `\n- ${cl.item} (${cl.checked ? 'Completed' : 'Pending'})`).join('') : 'No checklist items'}
`
    )).join('\n\n');
...
Company Knowledge:
${COMPANY_KNOWLEDGE}
Respond professionally and concisely without using markdown formatting like **, #, or *. Use plain text for lists and tables. For actions like create/update/delete, confirm the change.
`;
    // Call Grok API
