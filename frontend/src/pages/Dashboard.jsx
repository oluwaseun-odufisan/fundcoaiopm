    ...
    return (
        ...
        >
            <motion.div
                ...
                className="bg-white/95 backdrop-blur-lg rounded-xl p-4 sm:p-6 w-full max-w-xs sm:max-w-sm md:max-w-md border border-blue-200/50 shadow-xl"
            ...
            >
                ...
                <div className="space-y-2 sm:space-y-3">
                    <button
                        ...
                        className="w-full flex items-center gap-2 sm:gap-3 bg-green-100/50 text-green-700 px-3 sm:px-4 py-2 sm:py-3 rounded-lg hover:bg-green-200/70 transition-all duration-200 text-sm sm:text-base"
                    >
...
const Dashboard = () => {
    ...
    const STATS = [
        { key: 'total', label: 'Total', icon: Rocket, color: 'bg-blue-500/15 text-blue-600', valueKey: 'total', textColor: 'text-blue-700' },
        { key: 'completed', label: 'Done', icon: CircleCheck, color: 'bg-green-500/15 text-green-600', valueKey: 'completed', textColor: 'text-green-700' },
        { key: 'undone', label: 'Undone', icon: Layers, color: 'bg-amber-500/15 text-amber-600', valueKey: 'undone', textColor: 'text-amber-700' },
        { key: 'highPriority', label: 'High', icon: Flag, color: 'bg-indigo-500/15 text-indigo-600', valueKey: 'highPriority', textColor: 'text-indigo-700' },
        { key: 'mediumPriority', label: 'Medium', icon: Flag, color: 'bg-amber-500/15 text-amber-600', valueKey: 'mediumPriority', textColor: 'text-amber-700' },
        { key: 'lowPriority', label: 'Low', icon: Flag, color: 'bg-gray-500/15 text-gray-600', valueKey: 'lowPriority', textColor: 'text-gray-700' },
    ...
    return (
        <motion.div
            ...
            className="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-blue-100 flex flex-col font-sans"
        >
            <div className="flex-1 max-w-[1600px] mx-auto w-full px-8 py-12">
                <motion.div
                    ...
                    className="bg-white/95 backdrop-blur-lg border border-blue-100/50 rounded-3xl shadow-lg flex flex-col overflow-hidden"
                >
                    {/* Header */}
                    <header className="bg-blue-50/50 border-b border-blue-200/50 px-3 sm:px-4 md:px-6 py-3 sm:py-4 flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 shadow-sm">
                        <div className="flex items-center gap-2 sm:gap-3">
                            <Star className="w-6 h-6 sm:w-7 h-7 md:w-8 h-8 text-blue-600 animate-spin-slow flex-shrink-0" />
                            <div className="min-w-0">
                                <h1 className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold text-indigo-900 tracking-tight truncate">Task Board</h1>
                                <p className="text-xs sm:text-sm lg:text-base text-blue-600 tracking-tight line-clamp-1">Your Productivity Hub</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-2 sm:gap-3 md:gap-4">
                            <button
                                onClick={() => { setSelectedTask(null); setShowModal(true); }}
                                className="sm:hidden bg-white/95 text-blue-700 border border-blue-300/50 rounded-lg px-3 py-2 text-xs font-semibold hover:bg-blue-50 transition-all duration-300 shadow-sm hover:shadow-md flex items-center gap-1"
                            >
                                <Plus className="w-4 h-4" /> Task
                            </button>
                            <button
                                onClick={() => { setSelectedTask(null); setShowModal(true); }}
                                className="hidden sm:flex bg-white/95 text-blue-700 border border-blue-300/50 rounded-lg px-4 sm:px-5 py-2 sm:py-2.5 text-sm font-semibold hover:bg-blue-50 transition-all duration-300 shadow-sm hover:shadow-md flex items-center gap-2"
                            >
                                <Plus className="w-4 h-4 sm:w-5 h-5" /> Add Task
                            </button>
                            <div className="bg-white/95 border border-blue-300/50 rounded-lg px-3 sm:px-4 py-2 text-gray-800 text-xs sm:text-sm lg:text-base font-medium flex items-center gap-1 sm:gap-2 flex-shrink-0">
                                <Clock className="w-4 h-4 sm:w-5 h-5 lg:w-6 h-6 text-blue-600 animate-pulse flex-shrink-0" />
                                <span className="truncate">{currentTime.toLocaleTimeString('en-US', { hour12: true, timeZone: 'Africa/Lagos' })}</span>
                            </div>
                            <img
                                src={user?.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.name || 'User')}`}
                                alt="User Avatar"
                                className="w-8 h-8 sm:w-9 h-9 md:w-10 h-10 lg:w-12 h-12 rounded-full border-2 border-blue-400/50 hover:shadow-sm transition-all duration-200 flex-shrink-0"
                            />
                        </div>
                    </header>
                    ...
                    <main className="flex-1 flex flex-col overflow-hidden">
                        <div className="p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-6 lg:space-y-8">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3 sm:gap-4 max-w-[1400px] mx-auto">
                                {STATS.map(({ key, label, icon: Icon, color, valueKey, textColor }, index) => (
                                    <motion.div
                                        ...
                                        className="bg-white/95 backdrop-blur-md border border-blue-200/50 rounded-xl p-2 sm:p-3 md:p-4 lg:p-4 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 min-w-0"
                                    >
                                        <div className="flex items-center gap-2 sm:gap-3 md:gap-3 lg:gap-3">
                                            {key === 'completed' ? (
                                                <div className="relative w-8 h-8 sm:w-10 h-10 md:w-12 h-12 lg:w-12 h-12 flex-shrink-0">
                                                    ...
                                                    <PieChart className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-4 h-4 sm:w-5 h-5 md:w-6 h-6 lg:w-6 h-6 text-green-600" />
                                                </div>
                                            ) : (
                            ...
                            {/* Search and Sort */}
                            <div className="bg-white/95 backdrop-blur-md border border-blue-200/50 rounded-xl p-3 sm:p-4 md:p-5 shadow-sm">
                                <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 sm:gap-4">
                                    <div className="flex items-center gap-2 sm:gap-3 flex-1 w-full">
                                        <Search className="w-5 h-5 sm:w-6 h-6 md:w-7 h-7 text-blue-600 flex-shrink-0" />
                                        <input
                                            type="text"
                                            value={search}
                                            onChange={(e) => setSearch(e.target.value)}
                                            placeholder="Search tasks..."
                                            className="bg-white border border-blue-300/50 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-gray-800 focus:ring-2 focus:ring-blue-400 w-full transition-all duration-300"
                                        />
                                    </div>
                                    <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto">
                                        <ArrowUpDown className="w-5 h-5 sm:w-6 h-6 md:w-7 h-7 text-blue-600 flex-shrink-0" />
                                        <select
                                            value={sort}
                                            onChange={(e) => setSort(e.target.value)}
                                            className="bg-white border border-blue-300/50 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-blue-800 focus:ring-2 focus:ring-blue-400 transition-all duration-300 w-full sm:w-40 md:w-48"
                                        >
                                            {SORT_OPTIONS.map(({ value, label }) => (
                                                <option key={value} value={value} className="text-blue-800">
                                                    {label}
                                                </option>
                                            ))}
                                        </select>
                            ...
                            {/* Filters */}
                            <div className="bg-white/95 backdrop-blur-md border border-blue-200/50 rounded-xl p-3 sm:p-4 md:p-5 shadow-sm flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-4">
                                <div className="flex items-center gap-2 sm:gap-3">
                                    <Filter className="w-5 h-5 sm:w-6 h-6 md:w-7 h-7 text-blue-600 flex-shrink-0" />
                                ...
                                <div className="flex flex-wrap gap-2 sm:gap-3 w-full sm:w-auto">
                                    <select
                                        ...
                                        className="lg:hidden bg-white border border-blue-300/50 rounded-lg px-3 sm:px-4 py-2 text-sm sm:text-base text-blue-800 focus:ring-2 focus:ring-blue-400 transition-all duration-300 w-full"
                                    >
                                    ...
                                    <div className="hidden lg:flex flex-wrap gap-1 sm:gap-2 bg-blue-100/50 p-1 sm:p-2 rounded-lg">
                                        {FILTER_OPTIONS.map(opt => (
                                            <button
                                                key={opt}
                            ...
                            <div className="flex-1 overflow-y-auto custom-scrollbar">
                                <div className="bg-white/95 backdrop-blur-md border border-blue-200/50 rounded-2xl p-4 sm:p-5 md:p-6 shadow-sm">
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                                            {filteredTasks.length === 0 ? (
                                                <motion.div
                                                    initial={{ opacity: 0, y: 10 }}
                                                    animate={{ opacity: 1, y: 0 }}
                                                    className="col-span-full bg-white/95 backdrop-blur-md border border-blue-200/50 rounded-2xl p-6 sm:p-8 md:p-10 text-center shadow-md"
                                                >
                                                    <div className="w-16 h-16 sm:w-20 h-20 md:w-24 h-24 bg-blue-100/50 rounded-full flex items-center justify-center mx-auto mb-4 sm:mb-6">
                                                        <Layers className="w-8 h-8 sm:w-10 h-10 md:w-12 h-12 text-blue-600" />
                                                    </div>
                                                    ...
                                                    <p className="text-sm sm:text-base md:text-lg text-blue-600 mb-6 sm:mb-8 leading-relaxed line-clamp-2">
                                                        {filter === 'all' && !search ? 'Start your productivity journey with a new task!' : 'No tasks match your current filters.'}
                                                    </p>
                                                    <button
                                                        onClick={() => { setSelectedTask(null); setShowModal(true); }}
                                                        className="bg-blue-500 text-white px-6 sm:px-8 py-2 sm:py-3 rounded-lg flex items-center gap-2 sm:gap-3 mx-auto hover:bg-blue-600 transition-all duration-300 hover:scale-105 shadow-md hover:shadow-lg text-sm sm:text-base"
                                                    >
                                                ...
                                                filteredTasks.map((task, index) => (
                                                    <motion.div
                                                        ...
                                                        className={`relative bg-white/95 backdrop-blur-md border rounded-2xl shadow-md hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer ${
                                                            ...
                                                            task.priority?.toLowerCase() === 'high' ? 'border-indigo-600/50' :
                                                            task.priority?.toLowerCase() === 'medium' ? 'border-amber-300/50' :
                                                            task.priority?.toLowerCase() === 'low' ? 'border-gray-300/50' : 'border-blue-200/50'
                                                        }`}
                                                    ...
                                                    >
                                                        ...
                                                        <div className="absolute inset-0 bg-blue-500/10 opacity-0 hover:opacity-100 transition-opacity duration-300 rounded-2xl" />
                    ...
                    <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        onClick={() => { setSelectedTask(null); setShowModal(true); }}
                        className="fixed bottom-4 sm:bottom-6 right-4 sm:right-6 bg-blue-500 text-white p-3 sm:p-4 md:p-5 rounded-full shadow-md hover:bg-blue-600 transition-all duration-300 z-30"
                        title="Add New Task"
                    >
