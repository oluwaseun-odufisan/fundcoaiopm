...
const SlidePreview = ({ slide, theme, scale=1, isActive=false, onClick }) => {
  ...
  return (
    <div onClick={onClick} style={{ width:W*scale, height:H*scale, backgroundColor:`#${t.bg}`,
      ...
      {isTitle&&<div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',
      ...
      {isSection&&<div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',
      ...
      {isQuote&&<div style={{position:'absolute',inset:0,display:'flex',flexDirection:'column',
      ...
      {!isTitle&&!isSection&&!isQuote&&!isTwoCol&&<div style={{position:'absolute',inset:0,
...
const SlideEditor = ({ slide, onUpdate, onRegenerate, onDelete, onAddSlide, isRegenerating }) => {
  ...
  const inp = { backgroundColor:'var(--bg-subtle)', color:'var(--text-primary)',
...
const DeckPrep = () => {
  ...
  const uploadPDF=async(f)=>{
    ...
    }catch(e){ setError('Upload failed'); toast.error('Upload failed'); return null; }
