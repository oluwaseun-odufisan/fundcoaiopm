...
const PerformanceBoard = () => {
  ...
  if (isLoading) {
    return <div className="flex justify-center items-center h-screen text-blue-600 text-xl font-medium">Loading Performance Board...</div>;
  }
            ...
            {[1, 0, 2].map((pos, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`text-center cursor-pointer hover:shadow-md transition-all p-4 rounded-xl ${i === 1 ? 'order-last sm:order-first' : ''}`}
                ...
                >
                  ...
                  <h3 className="font-semibold text-base sm:text-lg">{performer.name}</h3>
                  <p className="text-blue-600 font-medium text-sm sm:text-base">{performer.points} pts</p>
                  <p className="text-xs sm:text-sm text-gray-600">{performer.level.name}</p>
          ...
          <div className="space-y-3">
            {leaderboard.slice(3, 10).map((u, i) => (
              <motion.div
                ...
                className="flex items-center justify-between p-4 bg-white rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer"
                onClick={() => setSelectedUser(u)}
              >
                <div className="flex items-center gap-3 sm:gap-4">
                  <span className="text-base sm:text-lg font-medium text-gray-600">#{i + 4}</span>
                  <div className="w-8 h-8 sm:w-10 h-10 rounded-full bg-blue-100 text-blue-600 flex items-center justify-center font-medium text-sm">
                ...
                <div className="text-right">
                  <p className="font-bold text-blue-600 text-base">{u.points} pts</p>
                  <p className="text-xs text-gray-500">{u.level.name}</p>
                </div>
              </motion.div>
            ))}
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {sortUsers().map(u => (
            <motion.div 
              ...
              className="bg-white rounded-3xl p-4 sm:p-6 shadow-md hover:shadow-xl transition-all cursor-pointer border border-gray-100"
              onClick={() => setSelectedUser(u)}
            >
              <div className="flex justify-between items-start mb-4 sm:mb-6">
                ...
                <div className={`px-2 py-1 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-medium ${
                  u.level.color === 'platinum' ? 'bg-indigo-100 text-indigo-800' :
                  u.level.color === 'gold' ? 'bg-yellow-100 text-yellow-800' :
                  u.level.color === 'silver' ? 'bg-gray-100 text-gray-800' :
            ...
            >
              <div className="flex justify-between items-start mb-6 sm:mb-8">
                <div>
                  ...
                  <p className="text-sm text-gray-600">{selectedUser.role} • Level {selectedUser.level.name}</p>
                </div>
                <button 
                  onClick={() => setSelectedUser(null)}
