...
const QuizComponent = ({ quiz, onComplete }) => {
  ...
  if (!timerRunning) {
    return (
      <motion.div 
        className="text-center py-8 bg-white rounded-lg shadow-inner"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      >
        <HelpCircle className="w-16 h-16 mx-auto mb-4 text-blue-300 animate-bounce" />
        <h3 className="text-lg font-bold text-blue-800 mb-4">Ready for Assessment?</h3>
        <p className="text-gray-600 mb-6">You have {quiz.length} questions. Time limit: {quiz.length} minutes. Hints available.</p>
        <button 
          onClick={startQuiz}
          className="bg-blue-600 text-white px-6 py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors flex items-center gap-2 mx-auto shadow-md"
        >
  ...
  return (
    <motion.div 
      className="bg-blue-50 p-6 rounded-xl mt-6 relative"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      aria-label="Quiz Assessment"
    >
      <div className="absolute top-4 right-4 bg-white px-3 py-1 rounded-full shadow text-blue-800 font-medium flex items-center gap-2 text-sm">
        <Clock className="w-4 h-4" /> {formatTime(timeLeft)}
      </div>
      <button 
        onClick={() => setShowHints(!showHints)}
        className="absolute top-4 left-4 text-blue-600 hover:text-blue-800 flex items-center gap-1 text-sm"
      >
      ...
      <AnimatePresence mode="wait">
        ...
        >
          <div className="space-y-6">
            <p className="font-medium text-blue-900 text-lg mb-4">{quiz[currentQuestion].question}</p>
            ...
            {showHints && quiz[currentQuestion].hint && (
              <motion.div 
                className="p-4 bg-yellow-50 rounded-lg text-sm text-yellow-800"
      ...
      <div className="flex justify-between mt-8">
        ...
        {currentQuestion < quiz.length - 1 ? (
          ...
          <button 
            ...
            className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-60 transition-colors"
          >
            Submit Assessment
          </button>
        )}
      ...
      {showAnswers && (
        ...
        >
          <h4 className="text-2xl font-bold text-blue-800 mb-4 text-center">Assessment Results</h4>
          <p className={`text-4xl font-bold text-center mb-6 ${score >= 80 ? 'text-green-600' : 'text-red-600'}`}>{score.toFixed(1)}%</p>
          <div className="space-y-6">
            {quiz.map((q, i) => (
              <div key={i} className="space-y-3 border-b pb-4 last:border-0">
                ...
                {q.options.map((opt, j) => (
                  ...
                  >
                    <span className="flex-1">{opt}</span>
                    {opt === q.answer && <CheckCircle className="w-5 h-5 text-green-600 ml-2" />}
                    {answers[i] === opt && opt !== q.answer && <XCircle className="w-5 h-5 text-red-600 ml-2" />}
                  </div>
                ))}
                {answers[i] !== q.answer && (
                  <p className="text-sm text-gray-600 mt-2 flex items-center gap-1">
                    <Lightbulb className="w-4 h-4 text-yellow-500" /> Explanation: {q.explanation || 'Review the module content for details.'}
                  </p>
                )}
              </div>
            ))}
