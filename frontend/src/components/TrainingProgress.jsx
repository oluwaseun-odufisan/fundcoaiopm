...
const TrainingProgress = ({ progress, courses, onViewCert }) => {
  ...
  const badges = [
    { level: 'Bronze', threshold: 25, color: 'text-amber-600' },
    { level: 'Silver', threshold: 50, color: 'text-gray-400' },
    { level: 'Gold', threshold: 75, color: 'text-yellow-500' },
    { level: 'Platinum', threshold: 100, color: 'text-blue-400' },
  ];
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-blue-50 p-4 rounded-lg text-center shadow-sm">
          <p className="text-3xl font-bold text-blue-800">{completedCourses.length}</p>
          <p className="text-sm text-gray-600">Completed Modules</p>
        </div>
        <div className="bg-blue-50 p-4 rounded-lg text-center shadow-sm">
          <p className="text-3xl font-bold text-blue-800">{earnedBadges.length}</p>
      ...
      <div className="mb-6">
        ...
        <div className="bg-blue-50 rounded-full h-3 overflow-hidden mb-2 shadow-inner">
          <motion.div 
        ...
        <p className="text-sm text-blue-600 text-right">{totalProgress.toFixed(1)}% Career Advancement</p>
      </div>
      <h3 className="text-lg font-semibold text-blue-800 mb-3 flex items-center gap-2">
          ...
          return (
            ...
            >
              <div className="flex justify-between mb-2">
                <p className="text-sm font-medium text-blue-800 truncate">{course.title}</p>
                <p className="text-sm text-blue-600">{courseProgress}%</p>
              </div>
              <div className="bg-white rounded-full h-2 overflow-hidden shadow-inner">
                <div className="bg-blue-600 h-full rounded-full" style={{ width: `${courseProgress}%` }}></div>
              </div>
              {courseProgress === 100 && (
                <button 
                  onClick={() => onViewCert(course)}
                  className="mt-2 text-xs text-blue-600 hover:underline flex items-center gap-1 w-full justify-center"
                >
