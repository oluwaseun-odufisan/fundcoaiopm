...
export const createTask = async (req, res) => {
    try {
        ...
        const { title, description, priority, dueDate, ownerEmail, checklist = [] } = req.body;
        if (!ownerEmail) {
...
export const approveTask = async (req, res) => {
    try {
        ...
        if (task.submissionStatus !== 'submitted') {
            return res.status(400).json({ success: false, message: 'Task not submitted for approval' });
        }
        ...
        console.error('Error approving/rejecting task:', err.message);
        res.status(400).json({ success: false, message: err.message });
    }
};
